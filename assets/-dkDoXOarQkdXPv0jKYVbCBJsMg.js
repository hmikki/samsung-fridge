;if(CloudflareApps.matchPage(CloudflareApps.installs['DGIayv0Aai50'].URLPatterns)){(function(){'use strict'
...
var options=window.CloudflareApps.installs['DGIayv0Aai50'].options
...
if("DGIayv0Aai50"==='preview')updateVisibility()}
function updateVisibility(){clearTimeout(visibilityTimeout)
var visible=true
var stored=false
try{stored=JSON.parse(store['cf-flashcard'])}catch(e){}
if("DGIayv0Aai50"==='preview'||!stored){visible=true}else{var timeframe=times[options.renew]
...
window.CloudflareApps.installs['DGIayv0Aai50'].scope={updateOptions:function updateOptions(nextOptions){clearTimeout(visibilityTimeout)
options=nextOptions
updateContent()
